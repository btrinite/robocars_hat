...
void _pwmdriver_set_throttle_output (int durationNs) {
  if (failsafe==0) {
#ifdef OUT_THROTTLE_TRIM
    if (durationNs<(PWN_out_throttle_Idle-PWN_out_throttle_DeadZone)) {
      durationNs = map(durationNs, 1000, PWN_out_throttle_Idle, PWN_out_throttle_Idle-PWN_out_throttle_MaxRev,PWN_out_throttle_Idle);    
    } else if (durationNs>(PWN_out_throttle_Idle+PWN_out_throttle_DeadZone)) {
      durationNs = map(durationNs, PWN_out_throttle_Idle, 2000, PWN_out_throttle_Idle,PWN_out_throttle_Idle+PWN_out_throttle_MaxFwd);    
    } else {
      durationNs = PWN_out_throttle_Idle;
    }
#endif
